...
class STCM2LDecompiler:
    ...
    def combine_dialogue_entries(self, entries: List[dict]) -> List[dict]:
        ...
        while i < len(entries):
            ...
            if current_type in (0x02, 0x03):
                ...
                if self.is_speaker_name(current_text):
                    ...
                    while i < len(entries):
                        ...
                        # v1.1.7: Only combine Type 0x04, 0x05, 0x06 dialogue entries (exclude 0x07 narration, handled separately)
                        if next_type not in (0x04, 0x05, 0x06):
                            break
                    ...
                    if text_len >= 2 and has_japanese and not self.is_bytecode(current_text):
                        ...
                        # Type 0x02/0x03 but not a speaker name (bytecode like "bg96", "suma", etc.)
                        # Skip it
                        i += 1
                        continue
                ...
                while j >= 0:
                    ...
                    if prev_type in (0x02, 0x03):
                        ...
                        if self.is_speaker_name(prev_text):
                            ...
                            break
                    elif prev_type in (0x04, 0x05, 0x06, 0x07, 0x12):
            ...
            # Handle Type 0x04, 0x05, 0x06 entries (dialogue only) - v1.1.7: exclude 0x07, 0x12 (handled separately)
            if current_type in (0x04, 0x05, 0x06):
                ...
                while j < len(entries):
                    ...
                    # v1.1.7: Only combine Type 0x04, 0x05, 0x06 dialogue entries (exclude 0x07, 0x0B, 0x0C, 0x12)
                    if next_type not in (0x04, 0x05, 0x06):
                        break
    ...
    def _parse_compact_format_OLD(self) -> List[dict]:
        """
        ...
        while pos < len(self.data) - 16:
            try:
                ...
                # Valid entry types are 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x0B, 0x0C
                # 0x05 and 0x07 are dialogue continuation types (v1.1.2)
                # 0x06 is also a dialogue type (v1.1.7)
                if entry_type not in [0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x0B, 0x0C]:
                    pos += 1
                    continue
...
def _parse_compact_format(self):
    """Wrapper to debug Type 6 entries."""
    import copy
    entries = self._parse_compact_format_OLD()
    type_6_entries = [e for e in entries if e.get('type') == 6]
    if type_6_entries:
        print(f"DEBUG: Found {len(type_6_entries)} Type 6 entries:", file=sys.stderr)
        for e in type_6_entries[:5]:
            print(f"  - Type={e['type']}, Index={e['index']}, Text="{e['text'][:40]}"", file=sys.stderr)
    return entries

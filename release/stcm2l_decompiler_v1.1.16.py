#!/usr/bin/env python3
"""
STCM2L Script Decompiler v1.1.16
...
Changelog:
    v1.1.16 - Added Type 0x0F support, fixed horizontal ellipsis terminal punctuation
        - Added Type 0x0F to valid types list and combining logic
        - Fixed horizontal ellipsis (… U+2026) not being treated as terminal punctuation
        - Fixed main() function to properly handle file paths ending with .txt
...
class STCM2LDecompiler:
    ...
    def combine_dialogue_entries(self, entries: List[dict]) -> List[dict]:
        ...
        while i < len(entries):
            ...
            if current_type in (0x02, 0x03):
                ...
                if self.is_speaker_name(current_text):
                    ...
                    while i < len(entries):
                        ...
                        if next_type not in (0x01, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F):
                            break
            ...
            # Handle Type 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E entries (dialogue only) - v1.1.10: exclude 0x12 narration
            # v1.1.15: Added 0x0D, 0x0E to type list
            if current_type in (0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E):
                ...
                while j >= 0:
                    ...
                    if self.is_name_indicator(prev_text):
                        ...
                        j -= 1
                    elif prev_type in (0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E):
                ...
                while j < len(entries):
                    ...
                    if next_type not in (0x01, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E):
                        break
    ...
    def _parse_padded_format(self) -> List[dict]:
        ...
        while pos < len(self.data) - 24:
            try:
                ...
                # Validate entry type (accept 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0F, 0x12)
                # 0x01: Choice options (English) (v1.1.13), 0x02/0x03: Choice options, 0x04: Main dialogue, 0x05/0x06: Dialogue continuation (v1.1.2), 0x07: Dialogue/narration, 0x08: Dialogue continuation (v1.1.10), 0x09: Dialogue (v1.1.11), 0x0A: Dialogue (v1.1.9), 0x0B/0x0C: Questions, 0x0F: Dialogue continuation (v1.1.15), 0x12: Narration
                if entry_type not in [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0F, 0x12]:
                    pos += 1
                    continue
                ...
                while pos < len(self.data) - 24:
                    ...
                    if (self.data[pos] == 0x00 and self.data[pos+1] == 0x00 and
                        ...
                        # v1.1.15: Added 0x0D, 0x0E to type list
                        if next_type in [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E] and 1 <= next_size <= 10000:
                            break
                    pos += 1
    ...
    def _parse_compact_format(self) -> List[dict]:
        ...
        while pos + 12 <= len(self.data):
            try:
                ...
                # Valid entry types are 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x12
                ...
                if entry_type not in [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x12]:
                    pos += 1
                    continue
    ...
    def decompile_full_format(self) -> List[dict]:
        ...
        for entry in compact_entries:
            ...
            if entry_index not in seen_indices:
                ...
                # Entry with this index already exists - APPEND the compact entry
                # This handles cases where multiple entries have the same binary index
                entries.append(entry)
                seen_indices.add(entry_index)

        # Add padded entries that don't overlap with compact entries
